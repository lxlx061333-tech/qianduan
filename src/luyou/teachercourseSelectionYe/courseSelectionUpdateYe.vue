...
const error = ref(new Array(4).fill(false))


axios({
      url: "/CourseSelection/queryScheduleId",
      params: {
        scheduleId: useroute.query.id,
      }
    }).then(req => {
      input.value[0] = formatDate(req.data.startTime)
      input.value[1] = formatDate(req.data.endTime)
      input.value[2] = req.data.status
...
function submitForm() {
  ...
  if (biao) {
    axios({
      url: "/CourseSelection/updateUpdate",
      params: {
        zhi: input.value
      },
      ...
      if (req.data === "yes") {
        alert("修改成功")
        userouter.push({
          path:'/teacherYe/teachercourseSelectionUpdate'
        })
      } else {
        alert("修改失败")
      }
