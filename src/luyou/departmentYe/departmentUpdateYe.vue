...
axios({
    method: 'get',
    url: '/Department/queryDepartmentId',
    params: {
      departmentId: useroute.query.id
    }
  }).then(req => {
    input.value[0] = req.data.departmentId
    input.value[1] = req.data.departmentName
    })
...
function fan(){
  userouter.push({
    path:'/guanliyuanYe/departmentUpdate'
  })
}
