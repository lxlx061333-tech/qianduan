...
console.log(useroute.query.teacherId)

axios({
  method: 'get',
  url: "/Teacher/queryTeacherId",
  params:({
    teacherId:useroute.query.teacherId
  })
}).then(req => {
  input.value[0] = req.data.teacherId,
  input.value[1] = req.data.title,
  input.value[2] = req.data.name,
  input.value[3] = req.data.departmentId,
  input.value[4] = req.data.phone,
  input.value[5] = req.data.email,
  input.value[6] = req.data.gender,
  input.value[7] = req.data.user.username,
  input.value[8] = req.data.user.password,
  zhanghao = req.data.user.username,
  gonghao = req.data.teacherId,
...
function submitForm(){
  ...
  const changes = []
  const fieldNames = ["工号","职称","名字","所属院系","手机号","邮箱","性别","账号","密码"]
  ...
  if(biao){
    ...
    return qs.stringify(params, { 
      ...
      if(req.data === "yes"){
        ...
        try {
          if(gonghao !== input.value[0]){
            yanGong();
            error.value[0] = true
            return
        }
        if(zhanghao !== input.value[7]){
         yanUsername()
         error.value[7] = true
         return 
        }else {
