...
axios({
  method: 'get',
  url: "/CourseSchedule/queryNotSelectionAll",
}).then(req => {
  data.value = req.data
...
function query() {
  axios({
    method: 'get',
    url: '/CourseSchedule/queryNameAndIdAndNotSelectionMoHu',
    params: {
      zhi: sou.value,
    }
  }).then(req => {
    ...
    if (req.data.length === 0) {
      userouter.push({
        ...
        query: {
          name: sou.value,
          yuan: '/teacherYe/teachercourseSelectionAdd'
        }
      })
    } else {
...
function submitForm(scheduleId,name){
    userouter.push({
    path:"/teacherYe/teachercourseSelectionAddYe",
    query:{
      id:scheduleId,
      name:name
    }
