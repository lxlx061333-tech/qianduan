<template>
  <div class="styi">
    <div v-show="xianshike">
      <span class="info-content" v-show="lk"> 当前课程都已经添加至选课表,请去添加课程 </span>
    </div>
        ...
        <div class="student-card">
          <div class="student-info">
            ...
            <div class="button-group">
              <button class="submit-btn" @click="submitForm(CourseSchedule.scheduleId,CourseSchedule.courseName)">
                <span>修改</span>
              </button>
...
axios({
  method: 'get',
  url: "/CourseSchedule/queryInSelectionAll",
}).then(req => {
  data.value = req.data
...
function query() {
  axios({
    method: 'get',
    url: '/CourseSchedule/queryNameAndId',
    params: {
      zhi: sou.value,
    }
  }).then(req => {
    ...
    if (req.data === '' || req.data.length === 0) {
      userouter.push({
        path: '/teacherYe/chano',
        query: {
          name: sou.value,
          yuan: '/teacherYe/teachercourseSelectionUpdate'
        }
      })
    } else {
...
function submitForm(scheduleId,name){
  userouter.push({
    path:"/teacherYe/teachercourseSelectionUpdateYe",
    query:{
      id:scheduleId,
      name:name
    }
